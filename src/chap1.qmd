...
# Unsorted
axes[0].imshow(Y, aspect='auto', cmap='Blues', interpolation='nearest')
axes[0].set_xlabel('Questions')
axes[0].set_ylabel('Models')
axes[0].set_title('Raw Response Matrix')

# Sorted
axes[1].imshow(Y_sorted, aspect='auto', cmap='Blues', interpolation='nearest')
...
Measurement requires a model connecting observable responses to latent constructs. This section surveys the major families of probabilistic models used in measurement: Item Response Theory, factor models, paired comparison systems, and network models. These models provide the statistical machinery for extracting latent variables from response data.
...
plt.title('Rasch Model: Item Characteristic Curves', fontsize=14)
...
4. **Multiple models exist for different purposes.** IRT models (1PL, 2PL, 3PL), factor models, paired comparison models (Bradley-Terry, Elo), and network models (GGM, Ising) serve different purposes. The choice has implications for what we can conclude.
...
- **Chapter 2 (Dataset Description):** Introduces the response matrices we will analyze---data from HELM, Open LLM Leaderboard, Chatbot Arena, and agent benchmarks.

- **Chapter 3 (Unidimensional Measurement):** Applies Rasch models to AI benchmarks, testing whether sum scores are appropriate summaries of model capability.

- **Chapter 4 (Multidimensional Measurement):** Uses factor models to uncover the dimensional structure of AI capabilities, revealing that aggregate scores hide important variation.

- **Chapter 5 (Prediction-Powered Evaluation):** Develops methods for efficient evaluation of new models and new questions using prediction models trained on historical data.

The measurement concepts from this chapter recur throughout. When we ask whether a benchmark is "valid," we mean validity in Borsboom's sense. When we justify using sum scores, we appeal to sufficiency in the Rasch sense. When we analyze benchmark dimensionality, we apply factor models from the psychometric tradition.
