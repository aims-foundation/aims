#!/usr/bin/env bash
# ────────────────────────────────────────────────────────────────────
# AIMS Chapter 2 — Stitch all animations into a single video
#
# Usage:
#   cd /lfs/skampere2/0/sttruong/aims
#   bash animations/ch2/stitch.sh                          # crossfade (default)
#   bash animations/ch2/stitch.sh --simple                 # hard-cut (faster)
#   bash animations/ch2/stitch.sh --music animations/music/chopin_nocturne_op9_no2.mp3
#
# Music options:
#   --music <file>       Path to a background music file (mp3/wav/flac)
#   --music-volume 0.12  Music volume level (0.0–1.0, default: 0.12)
#
# Output: animations/ch2/chapter2.mp4
# ────────────────────────────────────────────────────────────────────
set -euo pipefail
...
OUT="$ROOT/animations/ch2/chapter2.mp4"
TMPDIR="$(mktemp -d)"
...
CLIPS=(
    ...
    "likelihood_landscape/1080p60/LikelihoodLandscape.mp4"

    "section_titles/1080p60/Part2Title.mp4"
    "identifiability/1080p60/Identifiability.mp4"

    "section_titles/1080p60/Part3Title.mp4"
    "em_algorithm/1080p60/EMAlgorithm.mp4"

    "section_titles/1080p60/Part4Title.mp4"
    "bayesian_inference/1080p60/BayesianInference.mp4"

    "section_titles/1080p60/Part5Title.mp4"
    "cat_simulation/1080p60/CATSimulation.mp4"
...
for clip in "${CLIPS[@]}"; do
    ...
    if [[ ! -f "$full" ]]; then
        ...
        echo "  Run: manim -qh --disable_caching --media_dir media/ch2 animations/ch2/<file>.py <Scene>"
        exit 1
    fi
done
